...
struct BusStopEtaData: BusData, Codable{
    let type: String
    let version: String
    let generated_timestamp: String
    var data : [StopEtaData?] = []
    ...
    init(from decoder: Decoder) throws {
        ...
        do{
            let dataArray = try container.decodeIfPresent([StopEtaData].self, forKey: .data)
            self.data = dataArray!
        } catch{
            let dataDict = try container.decodeIfPresent(StopEtaData.self, forKey: .data)
...
struct StopEtaData: BusData , Codable, Identifiable{
    var id : String?
    let co : String?
    let route : String
    let dir : String
    ...
    init(from decoder: Decoder) throws {
        ...
        self.id = "\(route)_\(dir)_\(service_type)_\(String(seq))_\(dest_en)_\(String(describing: eta_seq))_\(String(describing: eta))_\(String(describing: rmk_en))"
